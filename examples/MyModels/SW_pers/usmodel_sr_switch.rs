...
parameterization
	...
	crpi(per,1)          ,	  1.488,1,2,normal_pdf(.9); % 1.488,1,2,normal_pdf(.9)
	crpi(per,2)          ,    0.488,0,1,normal_pdf(.9);
	cry(per,1)			 ,    0.1593,0.025,0.225,normal_pdf(.9); % 0.0593,0.025,0.225,normal_pdf(.9)
	cry(per,2)		     ,    0.0593,0.025,0.225,normal_pdf(.9);
	crhopinf(per,1)		,	0.8,0.5,0.9,beta_pdf(.9); % 0.8,0.3,0.7,beta_pdf(.9)
	crhopinf(per,2)  	,	0.4,0.2,0.6,beta_pdf(.9);
